...
namespace TeamProjectFrontEnd
{
    public partial class LoginPage2 : Form
    {
        ...
        {
            MariaDbConnAccount.MariaDbLib_Account dbLib = new MariaDbConnAccount.MariaDbLib_Account();
            ...
            {
                bool loginResult = dbLib.SelectDB(textBox2.Text+"@tilon.com", textBox3.Text);
                if (loginResult)
                {
        ...
        private void label5_Click(object sender, EventArgs e)
        {
            ...
            // 관리자 아이디일때(admin) -> 실행
            if (textBox2.Text == "admin" && !string.IsNullOrEmpty(textBox3.Text))
            {
                bool mLogin = dbLib.mLoginDB(textBox3.Text);
